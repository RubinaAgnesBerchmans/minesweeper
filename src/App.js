...
class App extends Component {
  ...
  initializeMatrixHandler = num => {
    ...
    let obj = { value: "", actValue: "", i: 0, j: 0 },
      i,
      j;
    let arr = Array(num)
      .fill()
      .map(() => Array(num).fill({ ...obj }));
  ...
  doComputation = (matrix, obj, event) => {
    ...
    if (!this.state.isBombPlaced) {
      ...
      console.log("TCL: App -> doComputation -> matrix", matrix);
    }
  };
  ...
  bombGenerateHandler = (matrix, obj, event) => {
    ...
    while (randomNoArr.length < 3) {
      ...
      if (
        !randomNoArr.includes(randomNum) &&
        randomNum !== obj.i &&
        randomNum !== obj.j &&
        randomNum < this.state.level
      ) {
  ...
  setHoverEmoji = () => {
    if (this.state.emojiState <= 2) this.setState({ emojiState: 1 });
  };
  unsetHoverEmoji = () => {
    if (this.state.emojiState <= 2) this.setState({ emojiState: 0 });
  };
  resetGame = () => {
    ...
    return (
      <React.Fragment>
        ...
        <div className="wrapper" id="wrapper">
          <button
            className="button"
            onClick={() => this.initializeMatrixHandler(8)}
          >
            Easy
          </button>
          <br />
          <button
            className="button"
            onClick={() => this.initializeMatrixHandler(9)}
          >
            Medium
          </button>
          <br />
          <button
            className="button"
            onClick={() => this.initializeMatrixHandler(10)}
          >
            Hard
          </button>
        </div>
        <div id="game-wrapper">
          <div className="upper-deck">
            <img className="icon" src={Dynamite} />
            <div className="iconName">6</div>
            <img
              className="emoji"
              src={
                this.state.emojiState === 0
                  ? Happy
                  : this.state.emojiState === 1
                  ? Think
                  : this.state.emojiState === 3
                  ? Sad
                  : Win
              }
            />
          ...
          <table
            id="table"
            className="table"
            onMouseOver={() => this.setHoverEmoji()}
            onMouseOut={() => this.unsetHoverEmoji()}
          >
            <tbody>
              {matrix.map(data => {
                return (
                  <tr className="tr">
                    {data.map(obj => {
                      return (
                        <td
                          className={
                            obj.actValue === "" && obj.value === "s"
                              ? "visited"
                              : obj.value === "*" && obj.actValue === "*"
                              ? "bomb"
                              : obj.actValue === obj.value && obj.value !== ""
                              ? "value"
                              : obj.actValue === "f"
                              ? "flag"
                              : "default"
                          }
                          onClick={event => this.setFlag(matrix, obj, event)}
                          onContextMenu={event =>
                            this.doComputation(matrix, obj, event)
                          }
                        >
                          {obj.value === "*" && obj.actValue === "*" ? (
